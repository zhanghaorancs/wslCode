...
def init_worker(data_path,train_data_path,test_data_path):
    ...
    worker_model = ResNet50Client(split_point=3)
...
if __name__ == '__main__':
    ...
    server_model = ResNet50Server(split_point=3).to(device)
    ...
    # for i in range(len(workers)):
        # rpc.rpc_sync(workers[i],init_worker,args=(data_path[i],train_data_path[i],test_data_path[i]))
    ...
    for epoch in range(epochs):
        ...
        print("第{}轮：第{}小轮：用时：{}s".format((epoch+1),(e+1),(time.time()-e_time)))
        with open("results/cifar10/test_splitpoint_3_local_epoch_test_loss_acc.txt", "a") as f:
            f.write(f"{epoch+1}\t{e+1}\t{loss.item()}\t{accuracy}\n")
        with open("results/cifar10/train_splitpoint_3_loss_acc.txt", "a") as f:
                f.write(f"{epoch+1}\t{e+1}\t{e_avg_loss}\t{e_avg_acc}\n")
