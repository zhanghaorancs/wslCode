# 不是随机取各个训练数据集，而是从头到尾遍历一次
# 同时还要考虑数据集的问题 --
    # 1.平均分配 
    # 2.Test时将各个客户端的模型执行平均聚合，然后再执行Test
...
def init_worker(data_path,train_data_path,test_data_path):
    global worker_model, worker_optimizer, worker_criterion, train_dataloader,test_dataloader, train_loader_iter,test_loader_iter
...
def worker_train_one_epoch(batch_idx):
    ...
    global worker_model, worker_optimizer, worker_criterion, train_dataloader, train_loader_iter,outputs,test_dataloader
    worker_model.train()
    if(batch_idx == 0):
        # 此时刚开始或者重新开始模型训练
        train_loader_iter = iter(train_dataloader)
...
epochs = 400
# 是train_loader的大小
# (len(dataset)/len(workers))/batch_size
local_epoch = 196
test_epochs = 20
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for e in range(local_epoch):
            ...
            futures = [rpc.rpc_async(worker, worker_train_one_epoch,args=(e,)) for worker in workers]
            # 等待所有异步调用完成并获取结果
            ...
            for output, target in results:
                global_train_outputs.append(output.clone().detach())
                global_train_targets.append(target.clone().detach())
                output_sizes.append(output.size(0))

            print("----------------------开始拼接train outputs和targets----------------------")
            # 此时需要拼接output和target
            train_outputs = torch.cat(global_train_outputs,dim=0).to(device)
            train_targets = torch.cat(global_train_targets,dim=0).to(device)
            ...
            for idx, worker in enumerate(workers):
                ...
                worker_target = global_train_targets[idx]
            ...
            with open("results/cifar10/train5010_sp1_s3_acc_loss.txt", "a") as f:
            ...
            for param, gradient in zip(server_model.parameters(), aggregated_gradient):
                if param.grad is None:
                    ...
                    param.grad.copy_(gradient.clone().detach())
        ...
        print("------------------异步调用客户端执行测试过程----------------")
        for test_e in range(test_epochs):
            # 异步调用客户端使用test_dataloader进行测试进行测试
            futures = [rpc.rpc_async(worker, worker_test_one_epoch,args=(test_e,)) for worker in workers]
        ...
        print("第{}轮：第{}小轮：用时：{}s".format((epoch+1),(e+1),(time.time()-e_time)))
        with open("results/cifar10/test5010_sp1_s3_acc.txt", "a") as f:
