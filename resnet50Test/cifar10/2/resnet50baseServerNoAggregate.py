...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for e in range(local_epoch):
            ...
            train_futures = [rpc.rpc_async(worker, worker_train_one_epoch,args=(e,)) for worker in workers]
            ...
            for output,target in train_results:
                ...
                # 记录损失和准确度
                worker_train_losses.append(loss.item())
                worker_train_accuracies.append(accuracy)

            e_avg_loss = sum(worker_train_losses) / len(worker_train_losses)
            e_avg_accuracy = sum(worker_train_accuracies) / len(worker_train_accuracies)
        ...
        with open("train_resnet5010_noAggregate_loss_accuracy.txt", "a") as f:
        ...
        print("------------------异步调用客户端执行测试过程----------------")
        # 随机选择一个客户端执行测试
        test_worker = random.choice(workers)
        test_loss, test_total, test_correct = 0.0, 0.0, 0.0
        for test_e in range(test_epoch):
            future = rpc.rpc_async(test_worker, worker_test_one_epoch,args=(test_e,))
        ...
        sequence_avg_loss = test_loss / test_epoch
        sequence_avg_accuracy = test_correct/test_total
        print("第{}轮：Test Accuracy: {},Test Accuracy: {}".format((epoch+1),sequence_avg_loss,sequence_avg_accuracy))

        print("第{}轮：第{}小轮：用时：{}s".format((epoch+1),(e+1),(local_epoch_end_time-local_epoch_time)))
        
        with open("results/cifar10/test_resnet5010_noAggreagte_loss_acc.txt", "a") as f:
