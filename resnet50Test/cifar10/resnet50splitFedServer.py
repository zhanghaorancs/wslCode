...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for e in range(local_epoch):
            ...
            # with open("results/cifar10/train_resnet5010_SFL_sp1_loss_acc.txt.txt", "a") as f:
            ...
            worker_models = [fut.wait() for fut in futures]
            # for idx, fut in enumerate(futures):
            #     fut.wait()
            
            print("--------------------接收各个客户端返回的本地模型执行本地模型同步--------------------")

            gradients = []
            for model in worker_models:
                model_grads = {name: param.grad.clone() for name, param in model.named_parameters() if param.grad is not None}
                gradients.append(model_grads)

            average_gradient = {}
            for key in gradients[0].keys():
                average_gradient[key] = torch.mean(torch.stack([grad[key] for grad in gradients]), dim=0)
            
            for name, param in worker_models[0].named_parameters():
                if param.grad is not None:
                    param.grad = average_gradient[name]

            # 将聚合后的模型发送至各个客户端
            futures = [rpc.rpc_async(worker, worker_model_update,args=(worker_models[0].state_dict(),)) for worker in workers]
            for idx, fut in enumerate(futures):
                fut.wait()
            ...
            # 打印损失
            # if(e % 5 == 0):
            #     print("第{}轮：第{}小轮：Train Loss: {}，Train Accuracy: {}".format((epoch+1),(e+1),e_avg_loss,e_avg_accuracy))
            # with open("train_loss_accuracy.txt", "a") as f:
            #     f.write(f"{epoch+1}\t{e+1}\t{e_avg_loss}\t{e_avg_accuracy}\n")
        ...
        with open("results/cifar10/test_resnet5010_SFL_sp1_loss_acc.txt", "a") as f:
