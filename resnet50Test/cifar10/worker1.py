...
data_path = "data/worker_data/cifar10/worker01"
train_data_path = "worker01_train_data.pkl"
test_data_path = "worker01_test_data.pkl"
def init_worker(data_path,train_data_path,test_data_path):
    ...
    print("dataset_size:{}".format(len(train_dataloader.dataset)))
    print("batch_size :{}".format(train_dataloader.batch_size))
    print("num_batches  :{}".format((len(train_dataloader.dataset) + train_dataloader.batch_size-1) // train_dataloader.batch_size))
    ...
    # worker_optimizer = optim.Adam(worker_model.parameters(), lr=0.0014)
...
def worker_test_one_epoch():
    ...
    global worker_model, worker_optimizer, worker_criterion, test_dataloader,outputs
    worker_model.eval()
    # if(batch_idx == 0):
    #     # 此时刚开始或者重新开始模型训练
    #     test_loader_iter = iter(test_dataloader)
...
if __name__ == '__main__':
    rpc.init_rpc("worker1",rank=1,world_size=5)
    print("---------------------------------{} RPC初始化---------------------------------".format(rpc.get_worker_info().name))

    rpc.shutdown()

    # init_worker(data_path,train_data_path,test_data_path)